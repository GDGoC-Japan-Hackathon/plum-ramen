...
@router.get("/api/diaries/{diary_id}/diary-questions-answer", response_model=GetFullDiaryDataResponse)
def get_full_diary_data(diaries_id: int, current_user: dict = Depends(get_current_user)):
    try:
        return get_full_diary_data_service(current_user["user_id"], diaries_id)
